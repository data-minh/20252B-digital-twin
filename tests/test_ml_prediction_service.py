...
from camera_device.camera_to_mqtt import encrypt_payload_aes_gcm
...
def test_decodes_camera_aes_gcm_payload():
    message = valid_message()
    key = b"0123456789abcdef"
    encrypted = encrypt_payload_aes_gcm(
        key,
        __import__("json").dumps(message).encode("utf-8"),
    )

    assert decode_mqtt_message(encrypted.encode("ascii"), key) == message
